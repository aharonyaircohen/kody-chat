 ...
 * @ai-summary A lesson is an ordered list of teaching steps that guide the
...
export const LESSON_ADVANCE_MODES = ["model", "keyword"] as const;
...
export const lessonConfigSchema = z
  .object({
    ...
    steps: z.array(lessonStepSchema).min(1).max(100),
  })
  .strict();
...
export function lessonPointerKey(slug: string): string {
  return `lesson:${slug}:step`;
}
