...
describe("users", () => {
  it("saves and upserts user-state per namespace and user", async () => {
    ...
    expect(
      await t.query(api.userState.get, { tenantId: REPO, namespace: "profile", userKey: "u2" }),
    ).toBeNull()
  })
  ...
  it("saves and upserts notification prefs per login", async () => {
    ...
    const prefs = await t.query(api.notificationPrefs.get, { tenantId: REPO, login: "octocat" })
  ...
  it("upserts inbox entries and lists newest first", async () => {
    ...
    const inbox = await t.query(api.inbox.list, { tenantId: REPO, login: "octocat" })
  ...
  it("saves and upserts channels-seen manifests", async () => {
    ...
    const seen = await t.query(api.channelsSeen.get, { tenantId: REPO, login: "octocat" })
