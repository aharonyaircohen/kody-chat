...
describe("intents", () => {
  ...
  it("appends and lists decisions in order per intent", async () => {
    ...
    expect(decisions.map((d) => d.decision.reason)).toEqual([
      "first",
      "second",
    ]);
  });
...
describe("intents schema enforcement", () => {
  it("keeps legacy policy records valid during migration", async () => {
    const t = setup();
    const canonical = validIntent();
    const { controls, policyRefs: _policyRefs, ...base } = canonical;
    await t.mutation(api.intents.save, {
      tenantId: TENANT,
      intentId: "legacy",
      intent: { ...base, id: "legacy", policy: controls },
      updatedAt: NOW,
    });
    expect(
