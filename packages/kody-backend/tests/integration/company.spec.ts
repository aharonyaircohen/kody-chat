...
describe("company", () => {
  it("saves and upserts intents", async () => {
    ...
    const intents = await t.query(api.company.listIntents, { tenantId: REPO })
  ...
  it("appends decisions in order per intent", async () => {
    ...
    expect(await run.query(api.company.listIntents, { tenantId: REPO })).toHaveLength(0)
  })
  ...
  it("saves and upserts goals", async () => {
    ...
    const goals = await t.query(api.company.listGoals, { tenantId: REPO })
  ...
  it("saves and upserts agents", async () => {
    ...
    const agents = await t.query(api.company.listAgents, { tenantId: REPO })
