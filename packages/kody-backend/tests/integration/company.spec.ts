...
describe("company", () => {
  it("saves and upserts intents", async () => {
    ...
    const intents = await t.query(api.intents.list, { tenantId: REPO })
  ...
  it("appends decisions in order per intent", async () => {
    ...
    expect(await run.query(api.intents.list, { tenantId: REPO })).toHaveLength(0)
  })
  ...
  it("saves and upserts goals", async () => {
    ...
    const goals = await t.query(api.goals.list, { tenantId: REPO })
  ...
  it("saves and upserts agents", async () => {
    ...
    const agents = await t.query(api.agents.list, { tenantId: REPO })
