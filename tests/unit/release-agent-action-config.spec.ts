...
describe("release agentAction config", () => {
  it("uses one release agentAction wired from the release agentResponsibility", () => {
    const agentResponsibility = readJson<{ agentAction: string; agentActions?: string[] }>(
      ".kody/agent-responsibilities/release/profile.json",
    );

    expect(agentResponsibility.agentAction).toBe("release");
    expect(agentResponsibility.agentActions).toBeUndefined();
    expect(existsSync(join(root, ".kody/agent-actions/release"))).toBe(true);
  });

  it("keeps the four release skills inside the single agentAction", () => {
    const profile = readJson<{
      name: string;
      inputs: Array<{ name: string; flag: string; type: string }>;
      claudeCode: { skills: string[] };
      scripts: { preflight: Array<Record<string, string>> };
    }>(".kody/agent-actions/release/profile.json");
  ...
  it("documents both single-main and dev-to-main branch policies", () => {
    const prompt = readText(".kody/agent-actions/release/prompt.md");
    const prepareSkill = readText(
      ".kody/agent-actions/release/skills/release-prepare/SKILL.md",
    );
