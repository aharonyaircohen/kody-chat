...
Sufficiency: purpose, agent, schedule, allowed commands, restrictions, optional agentActions, and optional inputs. Responsibilities produce structured evidence; the owning goal or loop writes Dashboard reports. Show the profile and body, then call \`create_or_update_agent_responsibility\` — the same tool handles both new agentResponsibilities and patches to an existing one (read-merge: omit a field to preserve it; pass \`body\` to replace the markdown; only call it after the user approves the diff).

**Key fields:** \`agent\` is the engine-aligned agentIdentity slug (the engine reads \`config.agent\`) \`agentActions\` (array) is for multi-run agentResponsibilities; \`agentAction\` (singular) is the convenience alias. Reports are generated by goals/loops after evidence is applied; do not add report settings or report paths to agentResponsibility bodies. \`profile\` (raw object) overrides any profile.json field the typed schema doesn't expose — use it for engine-specific keys, not as a substitute for the typed fields.`,
};
