...
const VAULT = "backend vault record";
...
const defaultBranch = () =>
  JSON.parse(sh(`gh api repos/${repo}`)).default_branch;

async function dispatchAndTime(branch, label) {
  const latest = () =>
    JSON.parse(
      sh(
        `gh run list --repo ${repo} --workflow kody.yml --limit 1 --json databaseId`,
      ),
    )[0]?.databaseId;
  const before = latest();
  sh(
  ...
  const buildSec = start && end ? Math.round((end - start) / 1000) : null;
