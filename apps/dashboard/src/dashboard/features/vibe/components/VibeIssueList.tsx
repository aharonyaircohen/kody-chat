...
import { cn, formatRelativeTime } from "@dashboard/lib/utils";
import { CIStatusBadge } from "@dashboard/lib/components/CIStatusBadge";
import { MiniPipelineProgress } from "@dashboard/lib/components/MiniPipelineProgress";
import {
  AlertCircle,
  GitPullRequest,
  Inbox,
    ...
    <div className="px-3 py-2.5 border-b border-white/[0.06] bg-black/30 sticky top-0 z-10 backdrop-blur-sm">
      <div className="relative">
        ...
        <Input
          ...
          className="h-auto w-full bg-white/[0.04] border border-white/[0.06] rounded-md pl-8 pr-7 py-1.5 text-xs text-zinc-200 placeholder:text-zinc-500 focus:outline-none focus-visible:ring-0 focus-visible:ring-offset-0 focus:border-white/20 focus:bg-white/[0.06] transition-colors"
        />
        {searchActive && (
          <Button
            type="button"
              ...
              >
                ...
                <div className="flex items-center gap-1.5 mt-1.5 min-w-0">
                  <Button
                    ...
                    className="text-[11px] tabular-nums font-medium text-sky-400 hover:text-sky-300 hover:bg-transparent underline decoration-sky-400/40 hover:decoration-sky-300 underline-offset-2 shrink-0 focus:outline-none focus-visible:ring-1 focus-visible:ring-sky-400/40 rounded px-0.5"
                  >
                    #{task.issueNumber}
                  </Button>
                  ...
                  {task.assignees && task.assignees.length > 0 && (
                    <div className="flex items-center -space-x-1.5 shrink-0 ml-auto">
                      ...
                      {task.assignees.length > 2 && (
                        <span
                          className="inline-flex items-center justify-center h-4 min-w-4 px-1 rounded-full bg-zinc-800 text-[8px] font-medium text-zinc-400 ring-2 ring-background"
