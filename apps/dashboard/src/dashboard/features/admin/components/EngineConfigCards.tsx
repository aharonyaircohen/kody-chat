...
} from "@dashboard/lib/engine/useEngineConfig";
import type { EngineEditableConfig } from "@dashboard/lib/api";
    ...
    <Card className="border-white/[0.08] bg-white/[0.03]">
      <CardContent className="p-4 space-y-3">
        ...
        >
          ...
          <code className="text-white/70">@kody build</code> run the{" "}
          <code className="text-white/70">run</code> capability.
        </CardHeader>
        {loading ? (
          <Loading />
        ) : (
          <>
            ...
            <div className="flex items-center gap-2">
              ...
              <Input
                ...
                placeholder="capability action (e.g. run)"
                disabled={saving}
                className="h-8 text-sm font-mono"
              />
              <Button
