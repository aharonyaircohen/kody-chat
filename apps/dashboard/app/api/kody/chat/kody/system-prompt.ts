 ...
 * Above this the agent uses `recall_search`, `list_memories`, and `recall`
 * against the tenant's Convex memory documents.
 */
...
The block below is the live contents of Convex \`instructions.md\` for this repo — the user's explicit preferences for how you should behave in this chat. These OVERRIDE the base agent prompt for tone, length, formatting, audience, and any other preference the user has chosen to record here. Apply them automatically; do not narrate that you're applying them.
...
export function buildSystemPrompt(
   ...
   */
    ...
    /**
     * Raw body of Convex `instructions.md` (or `null` when the file doesn't
    ...
    /**
     * Concatenated bodies of the `kody`-owned Convex `context/*.md` entries (or
    ...
    context?: string | null;
    /**
     * User-managed renderer rules compiled from Convex view renderers.
...
When the user asks to present structured records, statuses, choices, forms, or other data-shaped results, use \`show_view\` first. Keep explanations, analysis, diagnoses, and advice in \`final_answer\`.
...
${truncateMemoryContext(opts.memoryContext.trim())}`,
  ...
  if (opts?.report) {
    ...
    lines.push(
      `The user is viewing the report **${r.title}** (slug \`${r.slug}\`) on the dashboard's \`/reports\` page. Reports are Convex-backed diagnostic output produced by Kody capabilities and engine pipelines, never the source of truth for code.`,
    );
