 ...
 * @domain kody
 * @pattern company-backend-export-github-api
 * @ai-summary One-time migration path: exports the tenant's GitHub state-repo
 *   files as a DB-agnostic JSON dump keyed by backend table, using the shared
 *   mapStateFile mapping. The whole state repo is fetched as ONE tarball
 *   download instead of per-file REST reads (rate-limit rules in
 *   apps/dashboard/CLAUDE.md). Standing backups come from ../export (Convex).
 */
